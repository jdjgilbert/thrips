...
### FIGURE 5a and b
pdf('2016-01-14 Oocyte volume vs (a) nonrepro and (b) nest vol.pdf', width=10, height=6)
par(mfrow=c(1,2), family='serif')
with(subset(ndis), boxplot(log(oovol)~nrbin, las=1, names=c("Absent", "Present"), xlab='Nonreproductives', ylab=expression("Oocyte volume of reproductive foundresses ("*mm^{3}*"), log transformed")))
#legend('topleft', legend='', title='(a)', cex=2, bty='n')
mtext('(a)', 3, cex=2, line=0, padj=-0.5, adj=0)
with(subset(nest, females<8), plot(log(rep.oovol)~vol, las=1, xlab='Domicile volume', ylab=expression("Oocyte volume of reproductive foundresses ("*mm^{3}*"), log transformed")))
